...
class ClusterViewCompo extends React.Component {
  node: SVGSVGElement;
  this: any;
  ...
  componentDidMount() {
    const { node } = this;
    if (node) {
      this.initClusterView(node);
    }
  }

  initClusterView(nodeSvg) {

    const svg = d3.select(nodeSvg);
		const width = 1000; // get width in pixels
		const height = 1000;
    ...
    svg.style('background-color', '#eee');
      ...
      let legendOrdinal = d3_legend.legendColor()
			.scale(scaleColor)
			.shape('circle');
		...
		let legendSize = d3_legend.legendSize()
			.scale(sizeScale)
			.shape('circle')
			.shapePadding(10)
			.labelAlign('end');
